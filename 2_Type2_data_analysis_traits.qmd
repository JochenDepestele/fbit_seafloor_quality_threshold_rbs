---
title: "Exploration of longevity trait for Type 2 data"
subtitle: "Cumulative Biomass Distribution by longevity classes"
...
f_coeflmod <- function(L1, L1_3, L3_10, L10){ # Estimate slope and intercept of cumulative binomial distribution functions of the relative biomass distribution across longevity classes
...
f_predlong <- function(lintercept,lslope,prob=0.5){ # Predict the longevity for a relative cumulative biomass probabilitty
...
```{r Biomass-at-longevity by station}
...
B_dt[, (Lclasses) := lapply(.SD, function(x) x * as.numeric(biomass_g_per_sqm)), .SDcols = Lclasses]
...
B_dt <- B_dt[,
             ...
             .SDcols = Lclasses] # aphiaid is excluded to summarize across all species
# selcols <- c(".id","station_name","replicate","year") # Year is required to identify unique samples
# unique(B_dt[,..selcols])
# nrow(B_dt[rowSums(B_dt[,..Lclasses])==0]) # 4 records
B_dt <- B_dt[rowSums(B_dt[,..Lclasses])!=0] # The biomass in longevity classes cannot be zero
if(all(rowSums(B_dt[,..Lclasses])==0)){"Check that the total biomass in any sample is not zero"}

```

```{r slope and intercept of binomial distribution function}
...
B_dt[.id %in% "BoBIC_GulfofCadizsand_btrawling" & station_name == "arsa2015_4_"]$station_name <- "arsa2015_4"
