...
![Cumulative biomass distribution at longevity for each input dataset and sampling gears. Colors are indicative of the level of trawling disturbance, SAR except for the NS Silverpit study which used a trawling index.](./output/dist_pressure_vals_unique_pres.png){#fig-refpres_Q15 fig-pos="H"}
...
B_pred <- B_dt[pressure_type %in% c("SAR", "trawl_index") & !(.id %in% sel_id_multi_pres),
               {
...
# Step 4: Plot
# ---------------------------------------------------
ggplot(B_pred2[((pressure_type=="SAR" & pressure_value < ceiling(max_pres)+0.5) | 
                (pressure_type=="trawl_index"))], 
  ...
  geom_point(data = B_dt[pressure_type %in% c("SAR", "trawl_index") & 
...
#| tbl-cap: 'Slopes of pressure-state curves with associated pressures levels using longlived biomass thresholds'
#| tbl-pos: H
